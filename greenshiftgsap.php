...
define('GREENSHIFTGSAP_PLUGIN_VER', '3.4.3' );
...
if (!function_exists('greenShiftGsap_register_scripts_blocks')) {
	function greenShiftGsap_register_scripts_blocks()
	{
		...
		wp_register_script(
			'gsap-splinetool-runtime',
			GREENSHIFTGSAP_DIR_URL . 'libs/splinetool/runtime.js',
			array(),
			'0.9.486',
			true
		);
		...
		if (!is_admin()) {
			...
			if ($blockname === 'greenshift-blocks/animation-container') {
				...
				if ($initscript) {
					...
					wp_enqueue_script('gsap-splinetool-runtime');
				} else {
